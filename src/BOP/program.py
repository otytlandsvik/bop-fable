...
from fable_modules.fable_library.string_ import (to_console, interpolate)
...
def bus_ticket_price(age: int) -> None:
    to_console(((("Person of age " + str(age)) + " must pay ") + str(20 if (age < 18) else (40 if (age < 76) else 30))) + "kr")
...
to_console(interpolate("Eclidean distance between (2, 2) and (-3, -1) is %f%P()", [distance]))
...
def action(t: float) -> None:
    to_console(interpolate("Velocity at %.2f%P() was %.2f%P()", [t, velocity_at_time(t)]))
